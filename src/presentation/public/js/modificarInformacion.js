...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        document.getElementById('primerApellido').value = employeeData.primer_apellido || '';
        document.getElementById('segundoApellido').value = employeeData.segundo_apellido || '';
        document.getElementById('domicilioTrabajador').value = employeeData.domicilio || '';
    ...
    employeeForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetchWithToken(`http://localhost:3000/api/empleado/update-empleado/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (response.ok) {
                ...
                alert(result.errors ? result.errors.map(err => err.msg).join('\n') : `Error: ${result.message}`);
            }
        } catch (error) {
    ...
    // Evento para actualizar la contraseña del usuario
    employeeForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        try {
            const response = await fetchWithToken(`http://localhost:3000/api/empleado/update-password/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(filteredValues),
            });
            ...
            alert('Ocurrió un error al actualizar la contraseña del usuario.');
        }
    });

    // Evento para actualizar el correo del usuario
    employeeForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        try {
            const response = await fetchWithToken(`http://localhost:3000/api/empleado/update-email/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(filteredValues),
            });
            ...
            if (response.ok) {
                alert('Correo del usuario actualizado correctamente.');
                location.reload(); // Recargar la página para reflejar los cambios
            } else {
                console.error("Error en la actualización:", result);
            }
            ...
            alert('Ocurrió un error al actualizar el correo del usuario.');
        }
    });
});